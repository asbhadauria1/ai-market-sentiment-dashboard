...
def fetch_stock_history(ticker="AAPL", period="1y", interval="1d"):
    df = yf.download(ticker, period=period, interval=interval, group_by='ticker', auto_adjust=False)
    ...
    df = df[[c for c in expected_cols if c in df.columns] + ["Date"] if "Date" in df.columns else [c for c in expected_cols if c in df.columns]]
